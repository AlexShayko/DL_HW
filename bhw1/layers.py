...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        """
        # replace with your code ｀、ヽ｀、ヽ(ノ＞＜)ノ ヽ｀☂｀、

        if self.training:
            self.mean = np.mean(input, axis=0)
            self.var = np.var(input, axis=0)  # так как numpy по умолчанию считает смещенную дисперсию
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        if not self.training: # поскольку у нас running переменные будут константами, то градиенты считаются намного проще
            if self.affine:
        ...
        dx = (1.0 / N) * self.inv_sqrt_var * (N * dxhat - np.sum(dxhat, axis=0) - self.norm_input * np.sum(dxhat * self.norm_input, axis=0))
...
class Sequential(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        """
        # replace with your code ｀、ヽ｀、ヽ(ノ＞＜)ノ ヽ｀☂｀、ヽ
        grad_now = grad_output

        for i in range(len(self.modules) - 1, 0, -1):
            grad_now = self.modules[i].compute_grad_input(self.outputs[i - 1], grad_now)

        return self.modules[0].compute_grad_input(input, grad_now)
    ...
    def update_grad_parameters(self, input: np.ndarray, grad_output: np.ndarray):
        ...
        for i in range(len(self.modules) - 1, 0, -1):
            ...
            self.modules[i].update_grad_parameters(layer_input, grad_now)

            if i > 0:
                grad_now = self.modules[i].compute_grad_input(layer_input, grad_now)
        layer_input = input
        self.modules[0].update_grad_parameters(layer_input, grad_now)
