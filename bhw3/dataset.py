...
class TextDataset(Dataset):
    ...
    def __init__(self, data_file: str, train: bool = True, sp_model_prefix: str = None,
        ...
        """
        torch.manual_seed(self.TRAIN_VAL_RANDOM_SEED)
        all_indices = torch.randperm(len(texts))
        val_size = int(len(texts) * self.VAL_RATIO)
        val_indices = all_indices[:val_size]
        train_indices = all_indices[val_size:]
        val_texts = [texts[c] for c in val_indices]
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        bos = [self.bos_id]
        eos = [self.eos_id]
        encoded_text = bos + self.indices[item] + eos
        length = len(encoded_text)
        if length > self.max_length:
            encoded_text = encoded_text[:self.max_length - 1] + eos
            length = self.max_length
        else:
            encoded_text += [self.pad_id] * (self.max_length - length)
        indices = torch.tensor(encoded_text, dtype=torch.long)
        """
        YOUR CODE HERE (⊃｡•́‿•̀｡)⊃━✿✿✿✿✿✿
