...
def plot_losses(train_losses: List[float], val_losses: List[float]):
    ...
    """
    train_losses_tensor, val_losses_tensor = torch.tensor(train_losses), torch.tensor(val_losses)
    train_perplexities = torch.exp(train_losses_tensor).tolist()
    val_perplexities = torch.exp(val_losses_tensor).tolist()
...
def training_epoch(model: LanguageModel, optimizer: torch.optim.Optimizer, criterion: nn.Module,
    ...
    for indices, lengths in tqdm(loader, desc=tqdm_desc):
        ...
        indices = indices[:, :max_batch_len].to(device)

        inputs = indices[:, :-1].to(device)
        targets = indices[:, 1:].to(device)
        logits = model(inputs, (lengths - 1).cpu())

        flatten_logits = logits.view(-1, logits.size(-1))
        flatten_targets = targets.reshape(-1)

        loss = criterion(flatten_logits, flatten_targets)
        loss.backward()
        optimizer.step()
